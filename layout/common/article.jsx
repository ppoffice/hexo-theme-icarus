                ...
                <article class={`card-content article${'direction' in page ? ' ' + page.direction : ''}`}
                    ...
                    {page.layout !== 'page' ? <div class="article-meta size-small is-uppercase level is-mobile">
                        <div class="level-left">
                            {/* Date */}
                            <time class="level-item" dateTime={date_xml(page.date)}
                            ...
                            {page.categories && page.categories.length ? <span class="level-item">
                                {(() => {
                                    ...
                                    page.categories.forEach((category, i) => {
                                        categories.push(<a class="link-muted"
                    ...
                    <h1 class="title is-3 is-size-4-mobile">
                        {index ?
                            <a class="link-muted" href={url_for(page.link || page.path)}>{page.title}</a> : page.title}
                    </h1>
                    {/* Content/Excerpt */}
                    <div class="content"
                         dangerouslySetInnerHTML={{__html: getArticleContent(index, page, config)}}></div>
                    {/* Tags */}
                    ...
                    {/* "Read more" button */}
                    {index && isEnableExcerpt(page, config) ? <a class="article-more button is-small size-small"
                                                                 href={`${url_for(page.path)}#more`}>{__('article.more')}</a> : null}
                    {/* Share button */}
